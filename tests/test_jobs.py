...
def test_start_status_result_done(tmp_path):
    ...
    cwd = str(tmp_path)
    job_id, started = store.start(_factory(_WRITE_DONE), cwd, kind="delegate")
...
def test_cancel_removes_declared_external_paths(tmp_path):
    ...
    cwd = str(tmp_path)
    job_id, _ = store.start(_declare_factory(str(root)), cwd, kind="delegate")
...
def test_start_cleans_external_paths_if_meta_write_fails(tmp_path, monkeypatch):
    ...
    with pytest.raises(OSError):
        store.start(_declare_factory(str(root)), cwd, kind="delegate")
...
def test_activity_recorder_end_to_end_into_job_store(tmp_path: Path):
    """End-to-end: ActivityRecorder → activity.json → JobStore.status surfaces
...
def test_idempotent_first_start_creates_and_indexes(tmp_path):
    ...
    out = store.start_idempotent(
        _factory(_SLEEP), cwd, kind="k", tool="consult", key="k1", arg_hash="AH"
    )
...
def test_idempotent_same_key_different_args_conflicts(tmp_path):
    ...
    b = store.start_idempotent(
        _factory(_SLEEP), cwd, kind="k", tool="consult", key="k1", arg_hash="OTHER"
    )
...
def test_result_ok_true_for_stored_success(tmp_path):
    store = _store(tmp_path)
    cwd = str(tmp_path)
    job_id, _ = store.start(_factory(_WRITE_DONE), cwd, kind="delegate")
    assert _wait_terminal(store, cwd, job_id) == "done"
...
def test_result_ok_false_for_stored_error(tmp_path):
    store = _store(tmp_path)
    cwd = str(tmp_path)
    job_id, _ = store.start(_factory(_WRITE_ERROR), cwd, kind="consult")
    assert _wait_terminal(store, cwd, job_id) == "done"
